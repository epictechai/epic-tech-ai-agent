...
describe("todo-enforcer evaluate", () => {
	test("continues when unfinished native todos remain", () => {
		const decision = evaluate(ENABLED_CONFIG, makeState(), "Implemented part of it and still working.", true, [todo() as any])
	...
	test("continues even after a short conversational assistant turn if native todos remain", () => {
		const decision = evaluate(ENABLED_CONFIG, makeState(), "Yep, on it.", false, [todo() as any])
		expect(decision.action).toBe("continue")
	})

	test("stops when todos are complete", () => {
		const decision = evaluate(ENABLED_CONFIG, makeState(), "I finished everything.", false, [todo({ status: "completed" }) as any])
		expect(decision.action).toBe("stop")
		expect(decision.reason).toContain("all 1 items completed")
	})

	test("stops when all remaining todos are blocked", () => {
		const decision = evaluate(ENABLED_CONFIG, makeState(), "Still working.", true, [todo({ content: "waiting for credentials" }) as any])
		expect(decision.action).toBe("stop")

...
afterEach(() => {
	...
	describe("todo-enforcer plugin integration", () => {
		test("passive continuation is disabled by default", () => {
			expect(DEFAULT_CONFIG.features.continuation).toBe(false)
			expect(DEFAULT_CONFIG.features.todoEnforcer).toBe(false)
		})

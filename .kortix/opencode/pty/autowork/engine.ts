...
function buildPlanningPrompt(state: AutoworkState): string {
	const body = [
		`You are in the Machine autowork loop. Iteration ${state.iteration + 1}/${state.maxIterations}.`,
		"",
...
function buildExecutionPrompt(state: AutoworkState, approvedPlanOverride?: string | null, planningJustFinished = false): string {
	const body = [
		planningJustFinished
			? `Planning is complete. You are now entering the Machine autowork execution phase at iteration ${state.iteration + 1}/${state.maxIterations}.`
			: `You are in the Machine autowork execution phase. Iteration ${state.iteration + 1}/${state.maxIterations}.`,
		"",
...
function buildVerifierPrompt(state: AutoworkState, approvedCompletionOverride?: string | null): string {
	const body = [
		`You are in the Machine autowork verifier phase. Iteration ${state.iteration + 1}/${state.maxIterations}.`,
		"",

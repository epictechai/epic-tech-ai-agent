...
server**, auto-loaded into every session. It talks to the Machine **Executor
...
Machine dashboard. Each connector exposes **tools** (actions) with a connector-
...
session has the Executor wired (it always does in a Machine sandbox). All return
JSON.
...
- If `connectors` is empty or a tool is missing, the connector isn't configured
  or isn't **shared with this user**. Tell the user to add/share it in the Machine

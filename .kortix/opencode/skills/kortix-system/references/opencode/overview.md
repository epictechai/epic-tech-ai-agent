...
OpenCode is the agent runtime that powers every Machine session. The
same `.kortix/opencode/` config dir drives both surfaces:

- **Remote** — when a Machine session boots, the platform points OpenCode
...
## Where OpenCode looks for things in a Machine project

OpenCode discovers its config from `OPENCODE_CONFIG_DIR`, which the
Machine runtime sets to `.kortix/opencode/`. So everything below is
rooted there.

| Surface       | Path inside the Machine project                                                              |
| ------------- | ------------------------------------------------------------------------------------------- |
...
## The contract with Machine

OpenCode owns everything under `.kortix/opencode/`. The Machine platform
never reads any of it — those files only matter to OpenCode itself.

Conversely, Machine-specific config (triggers, secrets schema, sandbox
...
strict — see `references/kortix-toml.md` for the Machine half.

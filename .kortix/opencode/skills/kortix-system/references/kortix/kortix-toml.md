...
Machine platform reads about a project. It lives at the repo root.
Any repo with a valid `kortix.toml` at the root is a Machine project.
...
in the **Machine Secrets Manager** — never inline.
...
builder appends the Machine runtime layer on top of your image.
...
1. Declare the secret name under `[env]`:
   ...
   optional = ["ANTHROPIC_API_KEY", "OPENAI_API_KEY", "WEBHOOK_SLACK_SECRET"]
   ```
2. Set the value in the Machine Secrets Manager (dashboard).

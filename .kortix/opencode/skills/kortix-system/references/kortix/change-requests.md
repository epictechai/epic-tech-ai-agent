# Machine Change Requests — full reference

A **change request** (CR) is Machine's PR-equivalent. It proposes
merging one branch (`head_ref`) into another (`base_ref`) inside a
single Machine project. The CR layer is **Machine-native** — it works
...
Machine API against whatever backend the project's `repo_url` points
to.
...
| **Secrets**   | Decoupled. Secrets live in the Machine Secrets Manager, not the manifest; CRs don't move secrets. |
...
- **Merge is `Machine <noreply@kortix.ai>`-authored.** If you want the

# Machine CLI — full reference

The `kortix` CLI is the canonical way to drive everything the Machine
dashboard can do — from a terminal, from a coding agent, from a session
sandbox. It is **always available** inside a Machine session sandbox:
...
### Hosts — pick which Machine you talk to

A host is one Machine API endpoint. You can configure several
...
Machine-native PR layer for session work landing on `main`. A change
request proposes merging one branch (`head_ref`) into another
(`base_ref`) inside a project. The CR layer is **Machine-native** —

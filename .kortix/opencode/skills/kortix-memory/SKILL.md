...
<overview>
Every Machine project has a **project brain** at `.kortix/memory/` — a
...
same content. Edits land on `main` only via a Machine change request —
...
- Secrets, tokens, API keys, PII — those live in the Machine Secrets
...
only via a Machine change request — same path as any code change:
...
- **Don't put secrets in memory.** Memory is read into context and
  ...
  the Machine Secrets Manager, surfaced as env vars at runtime — not in
  `.kortix/memory/`.

...
- **Author** should default to `"Machine"` unless the user asks for a different organization or author name
- **Title** should be a descriptive name relevant to the document contents

Canvas API: `c.setTitle(...)`, `c.setAuthor("Machine")` right after creating the canvas.
SimpleDocTemplate: pass `title=...`, `author="Machine"` as constructor kwargs.
pdf-lib (JS): `doc.setTitle(...)`, `doc.setAuthor("Machine")`.

...
Always set metadata on every PDF you create. The author MUST be `"Machine"` and the title MUST describe the document contents.

- **Canvas**: call `c.setTitle(...)` and `c.setAuthor("Machine")` immediately after creating the canvas
- **SimpleDocTemplate**: pass `title=...` and `author="Machine"` as constructor kwargs

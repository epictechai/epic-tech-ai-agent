...
### Machine Design System (Fallback Palette — ONLY for Machine-branded sites)

This palette is ONLY for sites that are explicitly Machine-branded. For everything else, run the palette generator.

```css
/* NEXUS DESIGN SYSTEM — warm beige surfaces, teal primary accent */

:root, [data-theme="light"] {
  /* Surfaces (Machine Beige) */
  --color-bg:             #f7f6f2;
...
- `color-mix(in oklab, ...)` for opacity adjustments. Custom palettes: replace Machine defaults but keep variable names + both modes.
...
### Machine HSL Equivalents (for Tailwind / shadcn projects)

When using the fullstack webapp template (Tailwind + shadcn), `index.css` uses HSL values in `H S% L%` format (no `hsl()` wrapper). Below are Machine HSL conversions for when Machine is the appropriate fallback (see "Art Direction First" above). For inferred or custom palettes, convert your chosen colors to the same `H S% L%` format:

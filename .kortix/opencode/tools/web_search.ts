...
export default tool({
  ...
  async execute(args, _context) {
    ...
    // Prefer a direct Tavily key. If the box only has a proxy URL, use
    // KORTIX_TOKEN so the router can inject the platform key.
    const directApiKey = getEnv("TAVILY_API_KEY");
    const apiKey = directApiKey || (apiBaseURL ? getEnv("KORTIX_TOKEN") : undefined);
    if (!apiKey) return apiBaseURL
      ? "Error: KORTIX_TOKEN or TAVILY_API_KEY not set."
